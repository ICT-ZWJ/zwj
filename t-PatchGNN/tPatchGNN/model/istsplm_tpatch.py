...
class ISTS_tPatch(nn.Module):
    def __init__(self, args):
        ...
        # Learnable weights for weight_distortion aggregation (initialized to 0.5)
        self.gamma = nn.Parameter(torch.tensor(0.5), requires_grad=True)
        self.delta = nn.Parameter(torch.tensor(0.5), requires_grad=True)
        self.density_window = int(getattr(args, "density_window", 10))
        self.eps = 1e-6
        self.jitter = 1e-4
        ...
        # 3. Aggregation Components (Euclidean Main Path + SPD Auxiliary Path)
        # SPD Components (Auxiliary)
        self.spd_proj = nn.Linear(self.d_model, self.spd_dim * self.spd_dim)
        # Project logmap back to d_model to match Euclidean features for loss calculation
        self.spd_match_proj = nn.Linear(self.spd_dim * self.spd_dim, self.d_model)
        
        self.aux_loss = None
        self.aux_loss_weight = 0.1 # Weight for auxiliary SPD consistency loss
    ...
    def _compute_weight_distortion(self, tt, mask):
        ...
        distortion = self.gamma * irregularity + self.delta * density_fluct
        return torch.clamp(distortion, min=0.0, max=1.0)
    ...
    def forecasting(self, time_steps_to_predict, observed_data, observed_tp, observed_mask):
        ...
        # B. Auxiliary Path: SPD Constraint (Only during training)
        self.aux_loss = None
        if self.training:
            spd_seq = self._spd_from_features(seq_out)
            # Compute Frechet Mean in Tangent Space
            mean_log_spd = self._weighted_frechet_mean(spd_seq, weights) # [B*D, m, m] or None
            
            if mean_log_spd is not None:
                # Flatten and project to d_model
                flat_spd = mean_log_spd.view(B*D, -1)
                proj_spd = self.spd_match_proj(flat_spd) # [B*D, d_model]
                
                # Consistency Loss: Euclidean Aggregation should be close to Riemannian Aggregation
                consistency_loss = F.mse_loss(agg_feat, proj_spd.detach()) 
                
                self.aux_loss = consistency_loss * self.aux_loss_weight
