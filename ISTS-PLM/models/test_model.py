...
class istsplm_spd_forecast(nn.Module):
    def __init__(self, opt):
        ...
        self.tangent_proj = nn.Linear(self.spd_dim * self.spd_dim, self.d_model)
    ...
    def forecasting(self, time_steps_to_predict, observed_data, observed_tp, observed_mask):
        ...
        spd_tangent = torch.nan_to_num(spd_log).reshape(B, D, -1)

        var_feat = self.tangent_proj(spd_tangent)
